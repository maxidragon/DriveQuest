...
const Exam = () => {
    ...
    if (isFinished) {
        return (
            <div className="text-center text-2xl">
                ...
                {userAnswers.filter((userAnswer) => !userAnswer.answer?.isCorrect).map((userAnswer, index) => (
                   <AnsweredQuestionCard key={index} userAnswer={userAnswer} questions={questions}/>
                ))}
                </div>
            </div>
        );
    ...
    return (
        <div>
            <div>{t('timeLeft')}: {formatSeconds(timeLeft)} {currentQuestionIndex + 1}/32</div>
            <div className="flex flex-col gap-4 items-center text-center">
                <QuestionCard question={questions[currentQuestionIndex]} onAnswerClick={handleAnswer}/>
            </div>
        </div>
    );
};
